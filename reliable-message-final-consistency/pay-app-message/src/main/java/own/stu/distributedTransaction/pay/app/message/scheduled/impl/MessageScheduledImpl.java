...
package own.stu.distributedTransaction.pay.app.message.scheduled.impl;
...
import own.stu.distributedTransaction.pay.app.message.biz.MessageBiz;
...
import own.stu.distributedTransaction.pay.service.api.enums.MessageStatusEnum;
...
public class MessageScheduledImpl implements MessageScheduled {
    ...
    private Map<String, RpTransactionMessage> getMessageMap(int numPerPage, int maxHandlePageCount, Map<String, Object> paramMap) {
        ...
        PageBean pageBean = rpTransactionMessageService.listPage(paramMap);
        ...
        for (pageNum = 2; pageNum <= pageCount; pageNum++) {
            ...
            paramMap.put("pageNum", pageNum);
            paramMap.put("numPerPage", numPerPage);
            pageBean = rpTransactionMessageService.listPage(paramMap);
            recordList = pageBean.getRecordList();
            if (recordList == null || recordList.isEmpty()) {
                break;
            }
