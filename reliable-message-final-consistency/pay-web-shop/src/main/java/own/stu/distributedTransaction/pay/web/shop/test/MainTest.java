 ...
 * ====================================================================
 */
package own.stu.distributedTransaction.pay.web.shop.test;
...
import own.stu.distributedTransaction.pay.web.shop.utils.httpclient.SimpleHttpUtils;
...
//		listMerchantKey.add(new MerchantKey("ca6577dff6d647ac882dfb405ceda21e", "1b8da6c9b7544856955fcff6bf920f84"));//商户5 hpt
	...
	private static void testGateWayPay( String payKey , String paySecret){
		...
		for (int i = 0; i < 1; i++) { //每条线程生成数
			...
			String resultStr = SimpleHttpUtils.httpGet("http://127.0.0.1:8803/pay-web-gateway/scanPay/initPay", paramMap); // 请求网关创建订单
			...
			String notifyResultStr = SimpleHttpUtils.httpGet(PayConfigUtil.readConfig("pay-web-gateway")+ "/scanPayNotify/notify/TEST_PAY_HTTP_CLIENT", notifyMap);
